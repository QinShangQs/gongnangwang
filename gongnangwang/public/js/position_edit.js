...
function mySubmit(){
    if(par_positionVerify() & par_workVerify() & par_educationVerify() & par_ageVerify() & par_dutyVerify() & par_askVerify()){
        ...
        if((ok5 || ok6) & (ok7 || ok8)){
            $.ajax({
                type: "POST",
                url: "/position_edit_do",
                data: "pos_id="+pos_id+"&par_position="+par_position+"&par_work="+par_work+"&par_education="+par_education+"&par_age="+par_age+"&par_duty="+par_duty+"&par_pay_type="+par_pay_type+"&par_mode="+par_mode+"&par_mon="+par_mon+"&par_ann="+par_ann+"&par_return_type="+par_return_type+"&par_shares="+par_shares+"&par_other="+par_other+"&par_ask="+par_ask,
